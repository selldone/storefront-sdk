...
export class XapiAuthSMS extends APIAbstract {
  ...
  public verifyOTP(
    ...
    source: XapiAuth.LoginSource | null
  ): Promise<
  ...
  > {
    const params = {
      ...
      source: source ? source : XapiAuth.LoginSource.CUSTOMER,
    };
...
export interface ISMSVerifyOTPServerResponse_Select_User{
 id: number; name: string; email: string | number; code: string
}
